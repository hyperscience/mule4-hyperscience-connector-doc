...
This is the default authentication method for SaaS instances. 

In order to properly authenticate using this method follow these steps:

. Create an API account at Users > API Accounts.
. Copy the client ID and account password. You will not be able to access the account password after account creation.
. Use your client ID in the Client ID field.
. Use your account password in the Client Secret field
. For Auth Server, use the domain of the authentication server, i.e., login.hyperscience.net.
. For Domain, use the domain of the instance where HS app is hosted.
